...
function GoInFullscreen(element) {
      ...
      $.ajax({
          ...
          success(response){
            ...
            if(response.std == true) {
              getmenu(response.result[0].USERS_ZONE)
     ...
     * TODO : กลับไปหน้าเมนู
     */
    $('#btn-row-menu-back').on('click',() => {
      $('#row-doc').fadeOut("fast", () => {
        $('#row-menu').fadeIn("fast",() => {
     ...
     */
    $('#btn-row-login-back').on('click', async () => {
      await $('#row-menu').fadeOut("fast")
      await $('#row-login').fadeIn("fast")
