    ...
    $.ajax({
        ...
        async success(response){
            ...
            response.result.map((index ,current) => {
                 ...
                 * * if : ถ้าถึงข้อสุดท้ายจะแสดงผลปุ่ม ถัดไปที่สรุปรวมการตรวจสอบ แทนปุ่ม ถัดไป
                 */
                ...
                html += "           <button type=\"button\" id=\"btn-next-"+current+"\" onclick=\"vsr('"+countchoices+"')\" class=\"btn btn-md btn-success m-1\">ถัดไป</button>"
                } 
                else{
...
nextOrprevious = (_current,_numchoiceAll,_keyword) => {
    ...
    else if(_keyword == 'summary') {
        $('#card-summary').hide()
        $('#card-question-'+_numchoiceAll).show()
        // console.log(_numchoiceAll)
        Swal.mixin({
            toast: true,
            position: 'top-end',
            showConfirmButton: false,
            ...
            icon: 'success',
            title: 'กลับไปข้อก่อนหน้า : ' +_numchoiceAll
        })
    }
}